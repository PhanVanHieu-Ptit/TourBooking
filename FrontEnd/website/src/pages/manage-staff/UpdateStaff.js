    ...
    return (
        <form className='admin-form' onSubmit={handleSubmit}>
            <label style={{cursor: 'pointer'}} htmlFor='avt'>
                <img src={formData.imageUrl || svg.defaultAvt} alt='' />
            ...
            <label className={css['lock-account']} htmlFor='idStatus'>
                <input
                    ...
                    id='idStatus'
                    onChange={() => setFormData({...formData, idStatus: formData.idStatus == 5 ? 7 : 5})}
                />
                <label htmlFor='idStatus'>Khoá tài khoản</label>
            </label>
            <button className='btn--gold' style={{margin: '36px auto', display: 'block'}}>
                Lưu
            </button>
        </form>
    );
