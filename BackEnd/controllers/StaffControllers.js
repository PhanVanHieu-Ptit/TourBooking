...
class StaffController {
  ...
  async addStaff(req, res) {
    try {
      ...
      let [rows, fields] = await connection.execute(
        "call managetour.sp_create_staff(?, ?, ?);",
        [name, email, imageUrl]
      );
  ...
  async updateStaff(req, res) {
    try {
      ...
      let [rows, fields] = await connection.execute(
        "call managetour.sp_update_staff(?, ?, ?);",
      ...
      if (imageUrl)
        await connection.execute(
          "call managetour.sp_update_image_staff(?, ?);",
          [imageUrl, idStaff]
        );
      [rows, fields] = await connection.execute(
        `call managetour.sp_get_staff_by_id(${idStaff});`
      );
  ...
  async toggleAccountStatus(req, res) {
    try {
      ...
      [rows, fields] = await connection.execute(
        "call managetour.sp_update_status_staff(?, ?);",
        [newStatus, idStaff]
      );
      if (newStatus == 5)
