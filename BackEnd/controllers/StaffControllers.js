...
class StaffController {
    ...
    async addStaff(req, res) {
        try {
            ...
            await connection.execute('insert into account values (?,?,?)', [email, endcodedPassword, 'staff']);

            let [rows, fields] = await connection.execute('insert into staff(name,email,imageUrl) values (?,?,?)', [
                name,
                email,
                imageUrl,
            ]);
            ...
            if (rs.response.includes('OK')) {
                return res.send(message({idStaff: rows.insertId}, true, 'Thêm mới nhân viên thành công'));
            }
    ...
    async updateStaff(req, res) {
        try {
            ...
            let [rows, fields] = await connection.execute('update staff set name=?,idStatus=? where idStaff = ?', [
                name,
                idStatus || 5,
                idStaff,
            ]);
            ...
            if (imageUrl)
                await connection.execute('update staff set imageUrl=? where idStaff = ?', [imageUrl, idStaff]);
            [rows, fields] = await connection.execute(
                `select staff.*, status.name as status from staff inner join status on staff.idStatus = status.idStatus where idstaff=${idStaff}`,
            );
    ...
    async toggleAccountStatus(req, res) {
        try {
            const idStaff = req.params.id;
            let [rows, fields] = await connection.execute('select * from staff where idStaff = ?', [idStaff]);
            if (rows.length == 0) {
                return res.send(message(rows, false, 'Không tìm thấy nhân viên có id:' + idStaff));
            }
            ...
            [rows, fields] = await connection.execute('update staff set idStatus =? where idStaff = ?', [
                newStatus,
                idStaff,
            ]);
