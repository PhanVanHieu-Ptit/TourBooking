...
class AccountControllers {
    async signUp(req, res, next) {
        ...
        try {
            ...
            //insert tài khoản
            await connection.execute('insert into account(username,password) values(?,?)', [email, password]);
            ...
            return res.send(message({idCustomer: rows.insertId, username: email}, true, 'Đăng ký thành công'));
        } catch (error) {
    ...
    async signIn(req, res) {
        try {
            ...
            //check valid account
            let [rows, fields] = await connection.execute(`select * from account where username='${username}'`);
    ...
    async changePassword(req, res) {
        ...
        try {
            let [rows, fields] = await connection.execute('update account set password=? where username=?', [
                encodedPassword,
                username,
            ]);
