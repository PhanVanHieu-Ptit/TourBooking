...
async function tourInsert(req, res, next) {
  ...
  let { name, startDate, totalDay, tourIntro, tourDetail, pickUpPoint, tourDestination,
    price, minQuantity, maxQuantity, normalPenaltyFee, strictPenaltyFee,
    minDate, tourGuide, featured, tourPictures } = req.body;
  if (name.lenght < 10) {
  ...
  if (strictPenaltyFee < normalPenaltyFee) {
    return res.send(message({ normalPenaltyFee, strictPenaltyFee }, false, "Phí phạt nặng phải lớn hơn phí phạt thường!"));
  }
  if (minDate < 0) {
  ...
  if (!(await checkAddress(pickUpPoint))) {
    return res.send(message({ pickUpPoint }, false, "Điểm đón không hợp lệ!"));
  }
