...
CREATE TABLE IF NOT EXISTS `managetour`.`customer` (
  ...
  `name` VARCHAR(255) CHARACTER SET 'utf8mb3' NOT NULL COMMENT 'tên khách hàng',
...
CREATE TABLE IF NOT EXISTS `managetour`.`account` (
  ...
  CONSTRAINT `acc_cus`
    ...
    REFERENCES `managetour`.`customer` (`email`),
  CONSTRAINT `acc_staff`
    FOREIGN KEY (`username`)
    REFERENCES `managetour`.`staff` (`email`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;
